...
// Original columns: 65, Deployed columns: 64 (only 'Type' filtered out)
...
resource tenableiovulnclTable 'Microsoft.OperationalInsights/workspaces/tables@2025-02-01' = {
  parent: workspace
  ...
  properties: {
    ...
    schema: {
      name: 'Tenable_IO_Vuln_CL'
      description: 'Custom table Tenable_IO_Vuln_CL - imported from JSON schema'
      displayName: 'Tenable_IO_Vuln_CL'
      columns: [
        {
          name: 'TimeGenerated'
          type: 'dateTime'
...
output tableName string = tenableiovulnclTable.name
output tableId string = tenableiovulnclTable.id
output provisioningState string = tenableiovulnclTable.properties.provisioningState
