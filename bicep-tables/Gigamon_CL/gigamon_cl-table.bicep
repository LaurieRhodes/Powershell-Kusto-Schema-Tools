...
// Original columns: 52, Deployed columns: 51 (only 'Type' filtered out)
...
resource gigamonclTable 'Microsoft.OperationalInsights/workspaces/tables@2025-02-01' = {
  parent: workspace
  name: 'Gigamon_CL'
  properties: {
    plan: tablePlan
    ...
    schema: {
      name: 'Gigamon_CL'
      description: 'Custom table Gigamon_CL - imported from JSON schema'
      displayName: 'Gigamon_CL'
      columns: [
        {
          name: 'TimeGenerated'
          type: 'dateTime'
...
output tableName string = gigamonclTable.name
output tableId string = gigamonclTable.id
output provisioningState string = gigamonclTable.properties.provisioningState
