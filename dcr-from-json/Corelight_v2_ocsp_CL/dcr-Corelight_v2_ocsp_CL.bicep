    ...
    dataFlows: [
      {
        ...
        transformKql: 'source | project TimeGenerated = todatetime(TimeGenerated), _path_s = tostring(_path_s), _system_name_s = tostring(_system_name_s), _write_ts_t = todatetime(_write_ts_t), ts_t = todatetime(ts_t), id_s = tostring(id_s), hashAlgorithm_s = tostring(hashAlgorithm_s), issuerNameHash_s = tostring(issuerNameHash_s), issuerKeyHash_s = tostring(issuerKeyHash_s), serialNumber_s = tostring(serialNumber_s), certStatus_s = tostring(certStatus_s), revoketime_t = todatetime(revoketime_t), revokereason_s = tostring(revokereason_s), thisUpdate_t = todatetime(thisUpdate_t), nextUpdate_t = todatetime(nextUpdate_t)'
