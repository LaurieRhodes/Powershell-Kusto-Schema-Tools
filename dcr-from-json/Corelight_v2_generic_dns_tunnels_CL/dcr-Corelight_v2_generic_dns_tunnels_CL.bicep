    ...
    dataFlows: [
      {
        ...
        transformKql: 'source | project TimeGenerated = todatetime(TimeGenerated), _path_s = tostring(_path_s), _system_name_s = tostring(_system_name_s), _write_ts_t = todatetime(_write_ts_t), ts_t = todatetime(ts_t), dns_client_s = tostring(dns_client_s), domain_s = tostring(domain_s), bytes_d = toreal(bytes_d), capture_secs_d = toreal(capture_secs_d)'
