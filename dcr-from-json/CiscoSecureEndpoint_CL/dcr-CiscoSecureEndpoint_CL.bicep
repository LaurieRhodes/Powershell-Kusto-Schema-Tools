...
// Original columns: 86, DCR columns: 85 (Type column always filtered)
    ...
    dataFlows: [
      {
        ...
        transformKql: 'source | project TimeGenerated = todatetime(TimeGenerated), audit_log_id_g = tostring(audit_log_id_g), old_attributes_ip_external_s = tostring(old_attributes_ip_external_s), old_attributes_hostname_s = tostring(old_attributes_hostname_s), new_attributes_status_s = tostring(new_attributes_status_s), new_attributes_product_version_id_d = toreal(new_attributes_product_version_id_d), new_attributes_policy_id_d = toreal(new_attributes_policy_id_d), new_attributes_operating_system_id_d = toreal(new_attributes_operating_system_id_d), new_attributes_name_s = tostring(new_attributes_name_s), new_attributes_ip_external_s = tostring(new_attributes_ip_external_s), new_attributes_hostname_s = tostring(new_attributes_hostname_s), new_attributes_group_id_d = toreal(new_attributes_group_id_d), new_attributes_connector_guid_g = tostring(new_attributes_connector_guid_g), id_d = toreal(id_d), hostname_s = tostring(hostname_s), group_guids_s = tostring(group_guids_s), file_parent_process_id_s = tostring(file_parent_process_id_s), file_parent_process_id_d = toreal(file_parent_process_id_d), file_parent_identity_sha256_s = tostring(file_parent_identity_sha256_s), old_attributes_name_s = tostring(old_attributes_name_s), file_parent_identity_sha1_s = tostring(file_parent_identity_sha1_s), old_attributes_operating_system_id_d = toreal(old_attributes_operating_system_id_d), old_attributes_status_s = tostring(old_attributes_status_s), vulnerabilities_s = tostring(vulnerabilities_s), timestamp_nanoseconds_d = toreal(timestamp_nanoseconds_d), timestamp_d = toreal(timestamp_d), techniques_s = tostring(techniques_s), tactics_s = tostring(tactics_s), start_timestamp_d = toreal(start_timestamp_d), start_date_t = todatetime(start_date_t), severity_s = tostring(severity_s), scan_scanned_processes_d = toreal(scan_scanned_processes_d), scan_scanned_paths_d = toreal(scan_scanned_paths_d), scan_scanned_files_d = toreal(scan_scanned_files_d), scan_malicious_detections_d = toreal(scan_malicious_detections_d), scan_description_s = tostring(scan_description_s), scan_clean_b = tobool(scan_clean_b), RawData = tostring(RawData), orbital_version_s = tostring(orbital_version_s), orbital_old_version_s = tostring(orbital_old_version_s), old_attributes_product_version_id_d = toreal(old_attributes_product_version_id_d), file_parent_identity_md5_g = tostring(file_parent_identity_md5_g), file_parent_file_name_s = tostring(file_parent_file_name_s), file_parent_disposition_s = tostring(file_parent_disposition_s), computer_links_trajectory_s = tostring(computer_links_trajectory_s), computer_links_group_s = tostring(computer_links_group_s), computer_links_computer_s = tostring(computer_links_computer_s), computer_hostname_s = tostring(computer_hostname_s), computer_external_ip_s = tostring(computer_external_ip_s), computer_connector_guid_g = tostring(computer_connector_guid_g), computer_active_b = tobool(computer_active_b), Computer = tostring(Computer), command_line_arguments_s = tostring(command_line_arguments_s), cloud_ioc_short_description_s = tostring(cloud_ioc_short_description_s), cloud_ioc_description_s = tostring(cloud_ioc_description_s), bp_data_sts_d = toreal(bp_data_sts_d), bp_data_package_manager_serial_number_d = toreal(bp_data_package_manager_serial_number_d), bp_data_package_manager_pending_version_d = toreal(bp_data_package_manager_pending_version_d), audit_log_user_s = tostring(audit_log_user_s), audit_log_type_s = tostring(audit_log_type_s), audit_log_id_s = tostring(audit_log_id_s), computer_network_addresses_s = tostring(computer_network_addresses_s), computer_user_s = tostring(computer_user_s), connector_guid_g = tostring(connector_guid_g), created_at_t = todatetime(created_at_t), file_identity_sha256_s = tostring(file_identity_sha256_s), file_identity_sha1_s = tostring(file_identity_sha1_s), file_identity_md5_g = tostring(file_identity_md5_g), file_file_path_s = tostring(file_file_path_s), file_file_name_s = tostring(file_file_name_s), file_disposition_s = tostring(file_disposition_s), file_attack_details_suspicious_files_s = tostring(file_attack_details_suspicious_files_s), file_attack_details_base_address_s = tostring(file_attack_details_base_address_s), _ResourceId = tostring(_ResourceId), file_attack_details_attacked_module_s = tostring(file_attack_details_attacked_module_s), event_type_s = tostring(event_type_s), event_type_id_d = toreal(event_type_id_d), event_s = tostring(event_s), error_error_code_d = toreal(error_error_code_d), error_description_s = tostring(error_description_s), detection_s = tostring(detection_s), detection_id_s = tostring(detection_id_s), date_t = todatetime(date_t), file_attack_details_application_s = tostring(file_attack_details_application_s), _SubscriptionId = tostring(_SubscriptionId)'
