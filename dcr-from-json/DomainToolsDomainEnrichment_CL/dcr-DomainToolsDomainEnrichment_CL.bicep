...
// Original columns: 70, DCR columns: 69 (Type column always filtered)
    ...
    dataFlows: [
      {
        ...
        transformKql: 'source | project TimeGenerated = todatetime(TimeGenerated), TenantId = toguid(TenantId), IP_Country_s = tostring(IP_Country_s), IP_ISP_s = tostring(IP_ISP_s), MX_Host_s = tostring(MX_Host_s), MX_Domain_s = tostring(MX_Domain_s), MX_IP_s = tostring(MX_IP_s), Nameserver_Host_s = tostring(Nameserver_Host_s), Nameserver_Domain_s = tostring(Nameserver_Domain_s), Nameserver_IP_s = tostring(Nameserver_IP_s), Popularity_Rank_d = toreal(Popularity_Rank_d), Redirect_Domain_s = tostring(Redirect_Domain_s), Registrant_Name_s = tostring(Registrant_Name_s), Registrant_Org_s = tostring(Registrant_Org_s), Registrar_s = tostring(Registrar_s), SSL_Alt_Names_s = tostring(SSL_Alt_Names_s), IP_ASN_s = tostring(IP_ASN_s), SSL_Duration_s = tostring(SSL_Duration_s), SSL_Hash_s = tostring(SSL_Hash_s), SSL_Issuer_Common_Name_s = tostring(SSL_Issuer_Common_Name_s), SSL_Not_After_s = tostring(SSL_Not_After_s), SSL_Not_Before_s = tostring(SSL_Not_Before_s), SSL_Subject_s = tostring(SSL_Subject_s), SSL_Subject_Common_Name_s = tostring(SSL_Subject_Common_Name_s), SSL_Organization_s = tostring(SSL_Organization_s), Server_Type_s = tostring(Server_Type_s), Status_b = tobool(Status_b), TLD_s = tostring(TLD_s), Tags_s = tostring(Tags_s), Website_Title_s = tostring(Website_Title_s), Risk_Score_d = toreal(Risk_Score_d), Popularity_Rank_s = tostring(Popularity_Rank_s), SSL_Email_s = tostring(SSL_Email_s), IP_Address_s = tostring(IP_Address_s), Statcounter_Security_Codes_s = tostring(Statcounter_Security_Codes_s), Statcounter_Project_Codes_s = tostring(Statcounter_Project_Codes_s), SourceSystem = tostring(SourceSystem), MG = tostring(MG), ManagementGroupName = tostring(ManagementGroupName), Computer = tostring(Computer), RawData = tostring(RawData), Google_Analytics_d = toreal(Google_Analytics_d), Adsense_s = tostring(Adsense_s), Contact_Country_Code_s = tostring(Contact_Country_Code_s), Contact_Name_s = tostring(Contact_Name_s), Contact_Phone_s = tostring(Contact_Phone_s), Contact_Street_s = tostring(Contact_Street_s), Risk_Score_s = tostring(Risk_Score_s), Create_Date_s = tostring(Create_Date_s), Domain_s = tostring(Domain_s), Admin_Contact_Email_s = tostring(Admin_Contact_Email_s), Billing_Contact_Email_s = tostring(Billing_Contact_Email_s), SOA_Email_s = tostring(SOA_Email_s), Matomo_Codes_s = tostring(Matomo_Codes_s), Yandex_Codes_s = tostring(Yandex_Codes_s), Baidu_Codes_s = tostring(Baidu_Codes_s), Hotjar_Codes_s = tostring(Hotjar_Codes_s), Facebook_Codes_s = tostring(Facebook_Codes_s), GTM_Codes_s = tostring(GTM_Codes_s), _ResourceId = tostring(_ResourceId), Google_Analytics_4_s = tostring(Google_Analytics_4_s), First_Seen_t = todatetime(First_Seen_t), Expiration_Date_s = tostring(Expiration_Date_s), Email_Domain_s = tostring(Email_Domain_s), Whois_Email_s = tostring(Whois_Email_s), Technical_Contact_Email_s = tostring(Technical_Contact_Email_s), Registrant_Contact_Email_s = tostring(Registrant_Contact_Email_s), Google_Analytics_s = tostring(Google_Analytics_s), UTC = tostring(UTC)'
