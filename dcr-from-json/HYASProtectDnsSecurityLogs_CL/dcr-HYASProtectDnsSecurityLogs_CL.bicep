...
// Original columns: 57, DCR columns: 56 (Type column always filtered)
    ...
    dataFlows: [
      {
        ...
        transformKql: 'source | project TimeGenerated = todatetime(TimeGenerated), TenantId = toguid(TenantId), Nameserver2TLD_s = tostring(Nameserver2TLD_s), NameserverTLD_s = tostring(NameserverTLD_s), NameserverIP_s = tostring(NameserverIP_s), NameserverCountryISOCode_s = tostring(NameserverCountryISOCode_s), NameserverCountryName_s = tostring(NameserverCountryName_s), ARecord_s = tostring(ARecord_s), CName_s = tostring(CName_s), CName2TLD_s = tostring(CName2TLD_s), CNameTLD_s = tostring(CNameTLD_s), ThreatLevel_s = tostring(ThreatLevel_s), QueryType_s = tostring(QueryType_s), DomainTLD_s = tostring(DomainTLD_s), ResponseCode_d = toreal(ResponseCode_d), ResponseDescription_s = tostring(ResponseDescription_s), ResolverMode_s = tostring(ResolverMode_s), ReasonLists_s = tostring(ReasonLists_s), ReasonType_s = tostring(ReasonType_s), DomainAge_d = toreal(DomainAge_d), DomainCategory_s = tostring(DomainCategory_s), DomainCreationDate_t = todatetime(DomainCreationDate_t), DomainExpiresDate_t = todatetime(DomainExpiresDate_t), DomainUpdatedDate_t = todatetime(DomainUpdatedDate_t), NameserverVerdict_s = tostring(NameserverVerdict_s), FQDNVerdict_s = tostring(FQDNVerdict_s), ResponseName_s = tostring(ResponseName_s), DomainVerdict_s = tostring(DomainVerdict_s), Domain2TLD_s = tostring(Domain2TLD_s), ClientName_s = tostring(ClientName_s), SourceSystem = tostring(SourceSystem), MG = tostring(MG), ManagementGroupName = tostring(ManagementGroupName), Computer = tostring(Computer), RawData = tostring(RawData), IPVerdict_s = tostring(IPVerdict_s), TLDVerdict_s = tostring(TLDVerdict_s), Reputation_d = toreal(Reputation_d), DateTime_s = tostring(DateTime_s), Domain_s = tostring(Domain_s), DeviceName_s = tostring(DeviceName_s), ClientIP_s = tostring(ClientIP_s), ProcessName_s = tostring(ProcessName_s), Verdict_s = tostring(Verdict_s), VerdictSource_s = tostring(VerdictSource_s), VerdictStatus_s = tostring(VerdictStatus_s), Registrar_s = tostring(Registrar_s), PolicyName_s = tostring(PolicyName_s), PolicyID_d = toreal(PolicyID_d), RegistrarVerdict_s = tostring(RegistrarVerdict_s), TTL_d = toreal(TTL_d), Tags_s = tostring(Tags_s), LogID_s = tostring(LogID_s), ClientID_g = tostring(ClientID_g), Nameserver_s = tostring(Nameserver_s), _ResourceId = tostring(_ResourceId)'
